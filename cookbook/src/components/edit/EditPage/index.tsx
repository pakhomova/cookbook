...
class EditPage extends Component<IEditPageProps, IEditPageState> {
  ...
  componentDidUpdate = () => {
    this.props.submittingSuccess && this.props.history.push(routes.HOME);
  };
  ...
  render() {
    ...
    return (
      <>
        <Header headerText={this.state.isEditing ? 'Edit recipe ' : 'Add new recipe'} />
        <TextArea
          rows={15}
        ...
        />
        <LeftButton title="Submit" onClick={this.onButtonClick} disabled={this.isInputFieldEmpty()} />
      </>
    );
  }
  ...
  onButtonClick = (event: React.MouseEvent) => {
    this.state.isEditing
      ? this.props.postVersion(endpoints.EDIT_RECIPE + this.state.id, createDescriptionToPost(this.state.inputValue))
      : this.props.postVersion(endpoints.ADD_RECIPE, createDescriptionToPost(this.state.inputValue));
  };
  ...
  isInputFieldEmpty = () => !this.state.inputValue.trim();

  getInitialInputValue = (isEditing: boolean, id: string | undefined) => {
    if (isEditing) {
      let currentRecipe = this.props.recipesWithLastVersions.find((recipe: IRecipeWithLastVersion) => recipe.id === id);
