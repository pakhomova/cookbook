...
export const createDescriptionToPost = (description: string) => (
    {description}
);
...
export const mapRecipesToVersions = (recipes: RecipeFromGet[]) => {
    return recipes.map((recipe: RecipeFromGet) => (
        createVersion(recipe.dateCreated, recipe.description)
    ));
}

export const mapRecipesToRecipesWithLastVersions = (
    recipes: RecipeFromGet[]
) => {
    return recipes.map((recipe: RecipeFromGet) => {
